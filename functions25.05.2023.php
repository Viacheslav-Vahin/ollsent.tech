...
///
// Добавление поста
add_action('admin_post_create_custom_post', 'create_custom_post');
add_action('admin_post_nopriv_create_custom_post', 'create_custom_post');
function create_custom_post() {
    // Проверка nonce, CSRF защиты, и других элементов безопасности здесь
...
function delete_custom_post() {
    ...
    if(check_ajax_referer('delete_post_' . $post_id, 'nonce', false)) {
        $post_meta = get_post_meta($post_id, 'spec1', true);
        wp_delete_post($post_id, false);
        update_post_meta($post_id, 'spec1', $post_meta);
        echo 'success';
    } else {
        echo 'failure';
    }
...
//            $dataspecPost = get_page_by_title($datacontact, OBJECT, array('language', 'additional-lang', 'nontech'));
//            if ($dataspecPost) {
//                $meta_query[] = [
//                    'key' => 'spec1',
//                    'value' => $dataspecPost->ID,
//                    'compare' => '=',
//                ];
//            }
        }
    ...
    if (isset($_POST['filters']['zarpl']) && isset($_POST['filters']['zarpl_to'])) {
        $args['meta_query'][] = [
            'key' => 'zarplata',
            'value' => array($_POST['filters']['zarpl'], $_POST['filters']['zarpl_to']),
            'compare' => 'BETWEEN',
        ];
    }
