...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    ...
    if (message.action === 'saveCandidate') {
        ...
        console.log('Received checkCandidate message with lastName: ', message.lastName);
        getAuthToken()
            .then(authToken => {
            ...
            .then(candidateExists => {
                console.log('checkCandidate API response: ', candidateExists);
    ...
    //                 chrome.tabs.sendMessage(tabs[0].id, {action: 'checkCandidate', candidateExists: candidateExists});
    //             });
    //         })
    //         .catch(error => {
    //             console.error('Ошибка при проверке кандидата:', error);
...
async function saveCandidate(authToken, firstName, lastName, contacts = [], allInfo, foto, rezume, cvComb, candidateId = null) {
    ...
    let candidate;
    if (candidateId) {
        const response = await fetch(`https://crm.ollsent.tech/wp-json/wp/v2/candidate/${candidateId}`, {
            method: 'GET',
            headers: headers
        });
        if (!response.ok) {
    ...
    // }
    // else if(rezume.endsWith('.docx') || rezume.endsWith('.doc')) {
    //     let response_re = await fetch(rezume);
    //     let blobre = await response_re.blob();
    //     let formDatare = new FormData();
...
chrome.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
    if (changeInfo.status === 'complete' && /^http/.test(tab.url)) {
        console.log('onupdate back true');
