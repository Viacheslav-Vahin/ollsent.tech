...
// Проверка наличия сохраненных фильтров при загрузке страницы
    ...
    function filterCandidates() {
        ...
        console.log($('#opyt_vacans_input').val());
    ...
    `);
        ...
        window.addEventListener('storage', function(event) {
            if (event.storageArea === localStorage && event.key === 'filters') {
                filterCandidatesAndReplaceList();
            }
        });
        function filterCandidatesAndReplaceList() {
            let localStorageData = JSON.parse(localStorage.getItem('filters'));
            let filteredCandidates = candidates.filter(filterCandidatesByLocalStorage);
            let filteredCandidatesHTML = '';
            filteredCandidates.forEach(function(candidate) {
                filteredCandidatesHTML += '<div class="myCandW mcd kandItem1 baza" data-spec1="' + candidate.spec1 + '" data-oput="' + candidate.oput + '">' +
                    // Добавьте остальные необходимые данные для каждого кандидата
                    '</div>';
            });
            $('.allmr.pipeCont.list').html(filteredCandidatesHTML);
        }
        function filterCandidatesByLocalStorage(candidate) {
            let localStorageData = JSON.parse(localStorage.getItem('filters'));
            if (localStorageData.spec1 && localStorageData.spec1.length > 0) {
                if (!localStorageData.spec1.includes(candidate.spec1)) {
                    return false;
                }
            }
            if (localStorageData.oput && parseFloat(localStorageData.oput) > 0) {
                if (parseFloat(candidate.oput) <= parseFloat(localStorageData.oput)) {
                    return false;
                }
            }
            return true;
        ...
        $.ajax({
            ...
            success: function(response) {
                let candidates = [];
                $.each(response, function(index, postHtml) {
                    let kandItem = $(postHtml);
                    let candidate = {
                        spec1: kandItem.attr("data-spec1"),
                        oput: kandItem.data('oput')
                    };
