...
// Проверка наличия сохраненных фильтров при загрузке страницы
    $(document).ready(function() {
        setTimeout(function () {
            ...
            // Сохраняем значение в LocalStorage каждый раз, когда пользователь перемещает ползунок
            $('#opyt_vacans_input').on('input', function() {
                oput = $(this).val();
                // Сохраняем состояние фильтров после применения
            });
            if (savedFilters) {
                ...
                $('.spec1-checkbox').each(function () {
                    if (spec1Values.includes($(this).val().trim().replace(/[\-\[\]{}()*+?.,\\\^$|#\s]/g, "\\$&"))) {
    ...
    `);
        ...
        // Сохранение состояния фильтров после применения
        let filters = {
