    ...
    `);
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // Фильтруем кандидатов по всем параметрам
                // let filteredCandidates = candidates.filter(function(candidate) {
                //     let satisfiesOput = candidate.data('oput') > filters.oput;
                //     let satisfiesZarpl = candidate.data("zar") >= filters.zarpl && candidate.data("zar") <= filters.zarpl_to;
                ...
                //     let satisfiesRegExName = candidate.attr("data-spec1").match(filters.regExName);
                //     let satisfiesRegExAddName = candidate.attr("data-spec1").match(filters.regExAddName);
                //     let satisfiesNoTech = candidate.attr("data-spec1").match(filters.noTech);
                //     // let satisfiesSpec1 = filters.spec1.length === 0 || filters.spec1.some(spec => candidate.attr("data-spec1").match(new RegExp(spec, "ig")));
                //     // let satisfiesSpec1 = filters.spec1.length === 0 || filters.spec1.every(spec => candidate.attr("data-spec1").match(new RegExp(spec, "ig")));
                //     let satisfiesSpec1;
